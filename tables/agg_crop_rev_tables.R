...
mod5 <- readRDS("models/rev_crop_modten_1.rds")
mod6 <- readRDS("models/rev_crop_modtwenty_1.rds")
mod7 <- readRDS("models/rev_crop_modthirty_1.rds")
...
attr(mod7$beta, "dimnames")[[1]][1:10] <- c("weather_dday0_10", "weather_dday10_30", "weather_dday30", "weather_prec", "weather_prec_sq",
                                "climate_dday0_10", "climate_dday10_30", "climate_dday30", "climate_prec", "climate_prec_sq")
...
attr(mod7$coefficients, "dimnames")[[1]][1:10] <- c("weather_dday0_10", "weather_dday10_30", "weather_dday30", "weather_prec", "weather_prec_sq",
                                "climate_dday0_10", "climate_dday10_30", "climate_dday30", "climate_prec", "climate_prec_sq")


star1 <- stargazer(mod1, mod2, mod3, mod4, mod5, mod6, mod7,
                  align = FALSE, no.space = FALSE,
                  style = "aer", digits = 2,
                  omit = c("fips", "year", "state", "trend"),
                  omit.stat = c("ser", "f"),
                  title = "Regression Model explaining Crop Revenue per Acre",
                  column.labels = c("10-year", "10-year", "10-year", "10-year", "10-year", "20-year", "30-year"),
          ...
          add.lines = list(c("Fixed-effect", "--", "County", "County, Interval", "County, Interval", "County, Interval", "County, Interval", "County, Interval"),
                           c("Quad. State-trend", "--", "--", "--", "Yes", "Yes", "Yes", "Yes"),
...
star1 <- star1[1:loc-1]
star1notes <- paste("\\parbox{8in}{Notes: Table reports regression coefficients for log crop revenue per acre using weather (year-to-year) and climate (average of interval) degree day and precipitation variables from 1950-2010.
Crop revenue per acre is calculated by summing production (lbs) per acre times average crop price for corn, cotton, hay, soybean, and wheat from 1950-2010. Climate variables are averaged over intervals from 10 to 30-years. Regression
