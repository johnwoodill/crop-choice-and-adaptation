...
# Corn
star1 <- stargazer(corn1, corn2, corn3, corn4, corn5,
          ...
          add.lines = list(c("Fixed-effect", "--", "County", "County", "County", "County"),
                           c("Linear State-trend", "--", "--", "Yes", "Yes", "Yes"),
...
star1 <- star1[1:loc-1]
star1notes <- paste("\\parbox{4.3in}{Notes: Table reports regression coefficients for log corn revenue per acre using degree day variables and precipitation variables from 1950-2010. Corn revenue per acre is calculated by multiplying production (lbs) per acre times average crop price from 1950-2010. Regression estimates are weighted by total county-level corn acres (smoothed using a spline). Estimates in \\textbf{bold} are statistically significant at 95\\%. Coefficients have been multiplied by 100.}")
...
# cotton
star2 <- stargazer(cotton1, cotton2, cotton3, cotton4, cotton5,
                  align = FALSE, no.space = FALSE,
                  style = "aer", digits = 2,
                  omit = c("fips", "year", "state", "trend"),
                  omit.stat = c("ser", "f"),
...
# hay
star3 <- stargazer(hay1, hay2, hay3, hay4, hay5,
                  align = FALSE, no.space = FALSE,
                  style = "aer", digits = 2,
                  omit = c("fips", "year", "state", "trend"),
                  omit.stat = c("ser", "f"),
...
star3 <- star3[1:loc-1]
star3notes <- paste("\\parbox{4.3in}{Notes: Table reports regression coefficients for log hay revenue per acre using degree day variables and precipitation variables from 1950-2010. Hay revenue per acre is calculated by multiplying production (lbs) per acre times average crop price from 1950-2010. Regression estimates are weighted by total county-level hay acres (smoothed using a spline). Estimates in \\textbf{bold} are statistically significant at 95\\%. Coefficients have been multiplied by 100.}")
...
# soybean
star4 <- stargazer(soybean1, soybean2, soybean3, soybean4, soybean5,
                  align = FALSE, no.space = FALSE,
                  style = "aer", digits = 2,
                  omit = c("fips", "year", "state", "trend"),
                  omit.stat = c("ser", "f"),
...
star4 <- star4[1:loc-1]
star4notes <- paste("\\parbox{4.3in}{Notes: Table reports regression coefficients for log soybean revenue per acre using degree day variables and precipitation variables from 1950-2010. Soybean revenue per acre is calculated by multiplying production (lbs) per acre times average crop price from 1950-2010. Regression estimates are weighted by total county-level soybean acres (smoothed using a spline). Estimates in \\textbf{bold} are statistically significant at 95\\%. Coefficients have been multiplied by 100.}")
...
# wheat
star5 <- stargazer(wheat1, wheat2, wheat3, wheat4, wheat5,
                  align = FALSE, no.space = FALSE,
                  style = "aer", digits = 2,
                  omit = c("fips", "year", "state", "trend"),
                  omit.stat = c("ser", "f"),
...
star5 <- star5[1:loc-1]
star5notes <- paste("\\parbox{4.3in}{Notes: Table reports regression coefficients for log wheat revenue per acre using degree day variables and precipitation variables from 1950-2010. Wheat revenue per acre is calculated by multiplying production (lbs) per acre times average crop price from 1950-2010. Regression estimates are weighted by total county-level wheat acres (smoothed using a spline). Estimates in \\textbf{bold} are statistically significant at 95\\%. Coefficients have been multiplied by 100.}")
