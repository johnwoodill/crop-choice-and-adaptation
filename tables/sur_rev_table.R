...
skelmod <- lm(ln_rev ~ dday0_10 + dday10_30 + dday30 + prec + prec_sq - 1,
              data = regdat)
ncoef <- length(mod1$coefficients)
ncoef/5
...
star1 <- stargazer(skelmod, skelmod, skelmod, skelmod, skelmod, skelmod, skelmod,
                  ...
                  title = "Seemingly Unrelated Regression (SUR) Model explaining Crop Revenue/Acre",
                  column.labels = c("Corn", "Corn", "Corn", "Cotton", "Hay", "Soybean", "Wheat"),
          dep.var.labels = c("Log(Revenue/Acre)", "Log(Revenue/Acre)", "Log(Revenue/Acre)", "Log(Revenue/Acre)", "Log(Revenue/Acre)"),
          ...
          coef = list(skelmod = as.numeric(mod1$coefficients[2:6]),
                      ...
                      skelmod = as.numeric(mod4$coefficients[21:25])),
...
star1 <- star1[1:loc-1]
star1notes <- paste("\\parbox{5.5in}{Notes: Table reports regression coefficients for log crop revenue per acre using weather (year-to-year) degree days and precipitation variables from 1950-2010.
